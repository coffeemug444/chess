kernel void parallel_pad( global float* INPUT, 
   ...
   int input_elements = output_channel_elements*input_channels;

   int channel = (idx%input_elements) / output_channel_elements;

   int input = idx / input_elements;
   ...
   {
      OUTPUT[idx] = INPUT[input*input_elements + channel*input_channel_elements + row*input_width + col];
   }
}